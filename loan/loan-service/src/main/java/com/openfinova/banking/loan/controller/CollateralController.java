...
public class CollateralController {
    ...
    public ResponseEntity<CollateralResponse> registerCollateral(@PathVariable UUID loanAccountId,
        ...
        Collateral created = collateralService.registerCollateral(loanAccountId, collateral, "TODO_CURRENT_USER");
    ...
    public ResponseEntity<CollateralResponse> updateValuation(@PathVariable UUID loanAccountId, @PathVariable UUID id,
            @Valid @RequestBody CollateralValuationRequest request) {
    ...
    public ResponseEntity<CollateralResponse> updateCollateralStatus(@PathVariable UUID loanAccountId,
            @PathVariable UUID id, @Valid @RequestBody CollateralStatusUpdateRequest request) {

        Collateral collateral = collateralService
                .updateCollateralStatus(loanAccountId, id, request.getNewStatus(), "TODO_CURRENT_USER");
        return ResponseEntity.ok(CollateralMapper.toResponse(collateral));
    }

    @PostMapping("/{id}/release")
    @Operation(summary = "Release collateral from a loan")
    public ResponseEntity<CollateralResponse> releaseCollateral(@PathVariable UUID loanAccountId,
            @PathVariable UUID id) {

        Collateral collateral = collateralService.releaseCollateral(loanAccountId, id, "TODO_CURRENT_USER");
        return ResponseEntity.ok(CollateralMapper.toResponse(collateral));
    }
    ...
    public ResponseEntity<CollateralResponse> liquidateCollateral(@PathVariable UUID loanAccountId,
            @PathVariable UUID id, @Valid @RequestBody CollateralLiquidationRequest request) {

        Collateral collateral = collateralService
                .liquidateCollateral(loanAccountId, id, request.getLiquidationAmount(), "TODO_CURRENT_USER");
        return ResponseEntity.ok(CollateralMapper.toResponse(collateral));
    }
